...
- `OCR_TYPE`: `exact` (default) keeps the scanned image unchanged and only adds the text layer; `deskew` also straightens pages that were fed in crooked (the image is modified; applies to both OCR engines of the service). An invalid value falls back to `exact` with a message in the log
