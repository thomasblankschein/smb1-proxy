...
- **Adobe-Kontingent erschöpft:** Mit `OCR_FALLBACK=tesseract` in `.env.ocr` (OCR-Service ab v2.5.0) läuft die Texterkennung bei erschöpftem Adobe-Kontingent automatisch mit Tesseract weiter; im Proxy-Log steht dann `Ausweichen=adobe-quota`. Mit `OCR_ENGINE=tesseract` läuft sie dauerhaft ohne Adobe (keine Adobe-Zugangsdaten nötig). Die Qualität kommt dabei vor allem aus der LLM-Korrektur.
- **Schräge Scans:** Standard ist `OCR_TYPE=exact` (Originalbild bleibt unverändert). Werden Seiten schief eingezogen, kann `OCR_TYPE: deskew` beim `smb1proxy` unter `environment` die Seiten begradigen; das Bild wird dabei verändert (gilt für Adobe und für Tesseract). Danach `docker compose up -d`. Im Log steht der aktive Typ in der Startzeile.
